...
import { 
  ...
  Edit2, Trash2, Check, X // New Action Icons
} from 'lucide-react';
...
export default function App() {
        ...
        navigator.geolocation.watchPosition(
          (pos) => {
            ...
            socket.emit('update-location', { 
              userId, name: urlName, lat: latitude, lng: longitude, speed: speed 
            });
          }, 
          (err) => { setIsLocationGranted(false); }, 
          options
        );
      };
      startAutoTracking();
  ...
  const saveName = (userId) => {
    if (editName.trim()) {
      socket.emit('edit-user-name', { userId, newName: editName.trim() });
    }
    setEditingId(null);
  };

  const deleteUser = (userId) => {
    if (window.confirm("Are you sure you want to completely remove this tracking data?")) {
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {users.map(u => {
              ...
              return (
                <div 
                  key={u.userId} 
                  onClick={() => !isEditing && setSelectedUser(u)}
                  className={`p-3 md:p-4 rounded-xl mb-3 transition-all border ${selectedUser?.userId === u.userId ? 'bg-blue-600 border-blue-400 shadow-lg scale-[1.02]' : 'bg-slate-800 border-slate-700 hover:bg-slate-750 cursor-pointer'}`}
                >
                  <div className="flex justify-between items-center mb-2 gap-2">
                    <div className="flex items-center gap-2 flex-1 overflow-hidden">
                      <div className={`w-2 h-2 rounded-full flex-shrink-0 ${isOnline ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`}></div>
                      
                      {isEditing ? (
                        <div className="flex items-center gap-1 w-full bg-slate-900 rounded p-1">
                          <input 
                            type="text" 
                            value={editName}
                            onChange={(e) => setEditName(e.target.value)}
                            className="bg-transparent text-white text-xs w-full outline-none px-1"
                            autoFocus
                            onKeyDown={(e) => e.key === 'Enter' && saveName(u.userId)}
                          />
                          <button onClick={(e) => { e.stopPropagation(); saveName(u.userId); }} className="text-green-400 hover:text-green-300 p-1"><Check size={14}/></button>
                          <button onClick={(e) => { e.stopPropagation(); setEditingId(null); }} className="text-red-400 hover:text-red-300 p-1"><X size={14}/></button>
                        </div>
                      ) : (
                        <>
                          <span className="font-bold text-sm truncate flex-1" title={u.name}>{u.name}</span>
                          <button onClick={(e) => { e.stopPropagation(); startEditing(u); }} className="text-slate-400 hover:text-white p-1" title="Edit Name"><Edit2 size={12}/></button>
                          <button onClick={(e) => { e.stopPropagation(); deleteUser(u.userId); }} className="text-slate-400 hover:text-red-500 p-1" title="Delete Data"><Trash2 size={12}/></button>
                        </>
                      )}
                    </div>
                  ...
                  <div className="text-xs text-slate-400 mb-3 flex justify-between items-center">
                    ...
                    <span className="text-[10px] opacity-70">
                      {isOnline ? 'Live Now' : `${timeString} (${dateString})`}
                    </span>
                  </div>
    ...
    <div className="bg-white text-[#282c3f] font-sans antialiased min-h-screen">
        ...
        <div className="max-w-[1400px] mx-auto px-4 md:px-8 flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-6">
            <div className="hidden md:flex items-center bg-[#f5f5f6] rounded px-4 py-2 w-[400px] border border-[#f5f5f6] hover:bg-white hover:border-gray-200 transition-colors">
              ...
              <input type="text" placeholder="Search for products, brands and more" className="bg-transparent border-none outline-none w-full text-sm text-gray-700 placeholder-gray-500" />
            </div>
            <div className="flex items-center space-x-6 text-[#282c3f]">
              <div className="flex flex-col items-center cursor-pointer group"><User size={20} className="group-hover:text-[#ff3f6c] transition-colors" /><span className="text-xs font-bold mt-1">Profile</span></div>
              <div className="flex flex-col items-center cursor-pointer group"><Heart size={20} className="group-hover:text-[#ff3f6c] transition-colors" /><span className="text-xs font-bold mt-1">Wishlist</span></div>
              <div className="flex flex-col items-center cursor-pointer group relative"><ShoppingBag size={20} className="group-hover:text-[#ff3f6c] transition-colors" /><span className="text-xs font-bold mt-1">Bag</span><span className="absolute -top-1 -right-2 bg-[#ff3f6c] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">0</span></div>
            </div>
          </div>
        </div>
      </nav>
      ...
      <section className="w-full max-w-[1400px] mx-auto mt-6 px-4">
        <div className="relative w-full h-[400px] md:h-[500px] bg-gray-900 flex items-center justify-center overflow-hidden cursor-pointer group">
          <img src="https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=1600&q=80" alt="Fashion Sale" className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-700" />
          <div className="relative z-10 text-center text-white p-8 border-4 border-white/20 backdrop-blur-sm">
            ...
            <h1 className="text-6xl md:text-8xl font-black italic tracking-tighter mb-4 text-[#ff3f6c] drop-shadow-lg">50-80% OFF</h1>
            <p className="text-xl md:text-2xl font-bold tracking-widest mb-6">ON BIGGEST BRANDS</p>
            <button className="bg-white text-gray-900 px-8 py-3 font-bold uppercase tracking-widest hover:bg-[#ff3f6c] hover:text-white transition-colors duration-300">Shop Now</button>
          </div>
        </div>
      </section>

      <section className="w-full max-w-[1400px] mx-auto mt-16 px-4 pb-20">
        <h3 className="text-2xl font-bold text-center uppercase tracking-widest text-[#3e4152] mb-10">Shop By Category</h3>
      ...
      <section className="w-full bg-[#fde3f3] py-12 mt-10">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row items-center justify-between px-10">
          <div className="mb-6 md:mb-0"><h2 className="text-3xl font-black text-[#3e4152] uppercase mb-2">Myntra Insider</h2><p className="text-lg text-gray-700">Join the ultimate fashion loyalty program.</p></div>
          <button className="bg-[#ff3f6c] text-white font-bold px-10 py-4 rounded shadow-lg hover:bg-[#d62f55] transition-colors">ENROLL NOW</button>
        </div>
      </section>
      ...
      <footer className="w-full bg-[#fafbfc] border-t border-gray-200 pt-16 pb-8 mt-10">
        <div className="max-w-[1400px] mx-auto px-10 grid grid-cols-1 md:grid-cols-4 gap-8">
          <div><h4 className="font-bold text-xs text-[#282c3f] mb-4 tracking-widest">ONLINE SHOPPING</h4><ul className="space-y-2 text-sm text-[#696b79]"><li className="cursor-pointer hover:font-bold">Men</li><li className="cursor-pointer hover:font-bold">Women</li><li className="cursor-pointer hover:font-bold">Kids</li><li className="cursor-pointer hover:font-bold">Home & Living</li><li className="cursor-pointer hover:font-bold">Beauty</li><li className="cursor-pointer hover:font-bold">Gift Cards</li></ul></div>
          <div><h4 className="font-bold text-xs text-[#282c3f] mb-4 tracking-widest">CUSTOMER POLICIES</h4><ul className="space-y-2 text-sm text-[#696b79]"><li className="cursor-pointer hover:font-bold">Contact Us</li><li className="cursor-pointer hover:font-bold">FAQ</li><li className="cursor-pointer hover:font-bold">T&C</li><li className="cursor-pointer hover:font-bold">Terms Of Use</li><li className="cursor-pointer hover:font-bold">Track Orders</li><li className="cursor-pointer hover:font-bold">Returns</li></ul></div>
          <div className="col-span-1 md:col-span-2">
            <div className="flex gap-4 mb-6">
              <div className="flex items-center gap-2"><div className="w-10 h-10 bg-gray-200 rounded flex items-center justify-center font-bold">100%</div><span className="text-sm text-[#696b79]"><strong>ORIGINAL</strong> guarantee</span></div>
              <div className="flex items-center gap-2"><div className="w-10 h-10 bg-gray-200 rounded flex items-center justify-center font-bold">14</div><span className="text-sm text-[#696b79]"><strong>Return within 14days</strong></span></div>
            </div>
          </div>
        </div>
        <div className="max-w-[1400px] mx-auto px-10 mt-12 border-t border-gray-200 pt-6 text-center text-xs text-[#94969f]">© {new Date().getFullYear()} www.myntra.com. All rights reserved. A Flipkart company.</div>
      </footer>
    </div>
  );
}
